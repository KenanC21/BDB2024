...
animate_play_participation <- function(tracking_data, game_id, play_id)
{
  play <- tracking_data %>% 
    filter(gameId == game_id) %>% 
    filter(playId == play_id)
  ...
  anim <- football_field +
    geom_point(data = play, aes(x = x, y = y, color = class, group = nflId), alpha = 0.7,
               size = 6.5) +
    scale_color_manual(values = c("#e31837", "white", "#654321", "#002244"), guide = "none") +
    geom_text(data = play, aes(x = x, y = y, label = jerseyNumber), color = I(ifelse(play$class == "defense, no chance", "black", "white")),
              vjust = 0.36, size = 3.5) +
    transition_time(frameId) +
    theme(text = element_text(family = "Roboto"),
          axis.text = element_blank(),
