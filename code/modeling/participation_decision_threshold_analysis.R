...
tictoc::tic()
preliminary_model_data <- pmap_dfr(.l = list(1:9),
...
cutoff_plot <- cutoffs_long %>% 
  ggplot(aes(x = threshold, y = value, group = metric, color = metric)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Key Accuracy Metrics for Various Decision Thresholds",
       x = "Cutoff",
       y = "Value",
       color = "Metric")

ggsave("viz/decision_threshold_cutoff_plot.jpeg",
       width = 10,
       units = "in")
...
optimal_positive_cutoff_point <- positive_cutoff_points %>% 
  arrange(threshold) %>% 
  head(1) %>% 
  pull(threshold)

optimal_negative_cutoff_point <- negative_cutoff_points %>% 
  arrange(threshold) %>% 
  head(1) %>% 
  pull(threshold)

new_threshold <- min(optimal_negative_cutoff_point, optimal_positive_cutoff_point)

print(paste("New Threshold:", new_threshold))
...
pred_test_fact = factor(ifelse(pred_test >= new_threshold, 1, 0))
